...
export async function loadEmployerRegistry(root = process.cwd()): Promise<EmployerRegistryRecord[]> {
  ...
  for (const employer of employers) {
    ...
    if (employer.careersPage && employer.verification.status !== 'Verified') {
      throw new Error(`Employer ${employer.employerId} has a careers page without verified ownership evidence.`);
    }
